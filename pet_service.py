...
oauth2_scheme = OAuth2PasswordBearer(tokenUrl="/users/login")  #  Endpoint for login
...
def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    Returns:
        UserDB: The UserDB object representing the current user.
    ...
    try:
        ...
        username: str = payload.get("sub")  # "sub" claim contains the username
        if username is None:
